...
// Beatmap disk cache root. The original Windows default is an absolute 'D:\\' path;
// on non-Windows that backslash string is not a path separator and would create a junk
// literal-named folder, so default to the OS temp dir there. The desktop shell injects
// MINERADIO_BEAT_CACHE_DIR (per-user app data) on non-Windows; this default only applies
// to standalone server runs.
const BEATMAP_CACHE_DIR = process.env.MINERADIO_BEAT_CACHE_DIR
  || (process.platform === 'win32'
    ? 'D:\\MineradioCache\\beatmaps'
    : path.join(os.tmpdir(), 'MineradioCache', 'beatmaps'));
// Fallback installer asset extension by platform (used only when release metadata lacks
// an asset name). Windows ships .exe, macOS ships .dmg.
const DEFAULT_INSTALLER_EXT = process.platform === 'win32' ? 'exe' : 'dmg';
...
function normalizeManifestUpdateInfo(data) {
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `Mineradio-${latestVersion}-Setup.${DEFAULT_INSTALLER_EXT}`,
...
function beatCacheRootInfo() {
  ...
  // The C:-drive exclusion is a Windows-only policy (avoid caching on the system drive).
  // Off Windows there are no drive letters, so any resolvable root is allowed.
  const allowed = process.platform !== 'win32' ? !!root : (!!root && !/^C:$/i.test(drive));
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `Mineradio-${latestVersion}-Setup.${DEFAULT_INSTALLER_EXT}`;
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `Mineradio-${version || APP_VERSION}.${DEFAULT_INSTALLER_EXT}`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `Mineradio-${version || APP_VERSION}.${DEFAULT_INSTALLER_EXT}`;
}
...
function startUpdatePatchJob(info) {
  ...
  const job = {
    ...
    total: patch.size || 0,
    mode: 'patch',
    fileName: patch.name || safeUpdateFileName('', version).replace(/\.(exe|dmg|zip|appimage)$/i, '.patch.json'),
    filePath: '',
    version,
    downloadUrl,
    downloadCandidates,
