...
const APP_USER_MODEL_ID = 'com.lhysilicon.mineradio-macos';
...
const CHROMIUM_PERFORMANCE_SWITCHES = [
  ...
  // use-angle=d3d11 is Windows-only; on macOS Direct3D is absent, ANGLE fails to
  // create a GL backend and THREE.WebGLRenderer() throws, freezing the splash.
  // On non-Windows, fall back to Electron's default ANGLE backend (Metal on macOS).
...
ipcMain.handle('mineradio-open-update-installer', async (_event, filePath) => {
  try {
    ...
    // A Windows .exe installer cannot run on macOS/Linux; opening it would silently
    // do nothing. Surface an unsupported result instead.
    if (process.platform !== 'win32' && /\.exe$/i.test(target)) {
      return { ok: false, error: 'UPDATE_INSTALLER_UNSUPPORTED_PLATFORM', unsupported: true };
    }
...
ipcMain.handle('mineradio-wallpaper-set-enabled', async (_event, enabled, payload) => {
  // Live-wallpaper relies on the Windows WorkerW desktop re-parent (attachWallpaperToWorkerW,
  // win32-guarded). On macOS there is no equivalent, so creating the window only floats a
  // broken fullscreen panel. Report unsupported instead of spawning it.
  if (process.platform !== 'win32') return { ok: false, error: 'WALLPAPER_WINDOWS_ONLY', unsupported: true };
  try {
...
async function createWindow() {
  ...
  windowFullscreenActive = false;
  // Start the bundled local server only ONCE. On macOS the app stays alive after
  // all windows close (window-all-closed is a no-op on darwin), so createWindow()
  // runs again on dock-icon reopen; re-resolving findOpenPort here would pick a NEW
  // free port while the module-cached server stays bound to the original one,
  // yielding a blank window. Reuse mainServerPort when the server already exists.
  if (!localServer) {
    ...
    // Beatmap cache: server.js defaults to a hardcoded Windows 'D:\\' path. Only on
    // non-Windows route it into the per-user app data dir (mirrors the cookie/update
    // env wiring). Leave it UNSET on win32 so the original D:\\ + C:-drive gate logic
    // is preserved exactly.
    if (process.platform !== 'win32') {
      process.env.MINERADIO_BEAT_CACHE_DIR = process.env.MINERADIO_BEAT_CACHE_DIR
        || path.join(app.getPath('userData'), 'beatmaps');
    }
    try {
  ...
  await mainWindow.loadURL(`http://127.0.0.1:${mainServerPort}`);
}
...
if (!gotSingleInstanceLock) {
  ...
  app.whenReady().then(async () => {
    // macOS: install a minimal role-based application menu so standard shortcuts work
    // (Cmd+Q/H/M, and crucially Cmd+C/V/X/A/Z in the search & login text fields via the
    // Edit roles), while DELIBERATELY omitting the default menu's Reload / Force-Reload /
    // Toggle-DevTools items (Cmd+R would reload the renderer mid-session and reset player
