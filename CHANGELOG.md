...
## macOS fork (unofficial, by lhysilicon — modifications to XxHuberrr/Mineradio)

Unofficial macOS adaptation of XxHuberrr/Mineradio (GPL-3.0). Changed files and dates
...
- `desktop/main.js`: gate the Windows `use-angle=d3d11` GPU switch to win32 so macOS
  ...
  (`THREE.WebGLRenderer()` threw because Direct3D is absent).
- `server.js` + `desktop/main.js`: beatmap disk cache no longer defaults to a hardcoded
  Windows `D:\` path on macOS; routed to per-user app data / OS temp dir, and the
  C:-drive cache gate is skipped off-Windows. Windows behaviour unchanged.
- `desktop/main.js`: start the bundled local server only once and reuse its port on
...
- `desktop/main.js`: added a macOS-only role-based application menu (keeps Cmd+C/V/X/A
  in text fields; omits Reload/DevTools that would reset player state mid-session).
- `package.json`: added a macOS build target (`build:mac` → dmg/zip), music app
  category, neutral app icon; repointed the in-app updater / publish target and bundle
  id to this fork; kept productName "Mineradio" (nominative use, marked unofficial).
- `build/icon.png` + `build/icon.ico`: replaced the original MR-logo art with an
  independent neutral icon (the MR logo is the original author's reserved brand).
- Removed the original author's personal donation poster + `docs/SUPPORT.md` and the
  upstream Windows-installer brand images; README/NOTICE rewritten to mark this an
  unofficial macOS fork with attribution to upstream.
